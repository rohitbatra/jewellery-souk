...
class ControllerExtensionModuleSodeals extends Controller {
	public function index($setting) {
		...
		if ($use_cache){
			...
			if (!$_data) {
				$_data = $this->load->view('extension/module/so_deals/'.$data['store_layout'].'.tpl', $data);
			...
			return $this->load->view('extension/module/so_deals/'.$data['store_layout'].'.tpl', $data);
		}
	}
	...
	private function processCategory($catids)
	{
		...
		foreach($catids as $category_item)
		{
			...
			{
				if($checkCategory[0]['status'] == 1) $category_list[] =  $category_item;//dev
