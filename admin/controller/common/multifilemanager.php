...
class ControllerCommonMultifileManager extends Controller {
	public function index() {
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => $this->url->link('common/multifilemanager', 'token=' . $this->session->data['token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'))) . $url, 'SSL')
				);
		...
		$data['parent'] = $this->url->link('common/multifilemanager', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$data['refresh'] = $this->url->link('common/multifilemanager', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$pagination->limit = 16;
		$pagination->url = $this->url->link('common/multifilemanager', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$this->response->setOutput($this->load->view('common/multifilemanager.tpl', $data));
	}
	...
	public function upload() {
		...
		if (!$this->user->hasPermission('modify', 'common/multifilemanager')) {
