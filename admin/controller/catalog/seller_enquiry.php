<?php
class ControllerCatalogSellerEnquiry extends Controller {
    private $error = array();

    public function index() {
        $this->load->language('catalog/seller_enquiry');

        $this->document->setTitle($this->language->get('heading_title'));

        $this->load->model('catalog/seller_enquiry');
    ...
    public function view() {
        ...
        if($this->user->isSeller()) {
            $this->model_catalog_seller_enquiry->markEnquiryAsRead($this->request->get['enquiry_id']);
        }

        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_catalog_seller_enquiry->saveReplyToDb(array('reply_content' => $this->request->post['reply_content'], 'enquiry_id' => $this->request->get['enquiry_id']));
            $this->model_catalog_seller_enquiry->sendReplyToEnquirer(array('reply_content' => $this->request->post['reply_content'], 'enquiry_id' => $this->request->get['enquiry_id']));

            $this->session->data['success'] = $this->language->get('text_success');

            $this->response->redirect($this->url->link('catalog/seller_enquiry', 'token=' . $this->session->data['token'], true));
        }
    ...
    protected function getForm() {
        ...
        $data['text_seller_enquiry'] = $this->language->get('text_seller_enquiry');
        $data['text_reply_seller_enquiry'] = $this->language->get('text_reply_seller_enquiry');
        ...
        $data['action'] = $this->url->link('catalog/seller_enquiry/view', 'token=' . $this->session->data['token'] ."&enquiry_id=".$this->request->get['enquiry_id'], true);

        $data['cancel'] = $this->url->link('catalog/seller_enquiry', 'token=' . $this->session->data['token'], true);

        // Get Enquiry
        $this->load->model('catalog/seller_enquiry');
        if (isset($this->request->get['enquiry_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $enquiry_info = $this->model_catalog_seller_enquiry->getEnquiry($this->request->get['enquiry_id']);
        }
        ...
        if(isset($enquiry_info['seller_id']) && $data['is_seller'] == false) {
        ...
        $data['enquiry_added'] = $enquiry_info['dataTime'];
        ...
        $this->response->setOutput($this->load->view('catalog/seller_enquiry_form', $data));

    }

    protected function validateForm() {
        if (!$this->user->hasPermission('modify', 'catalog/seller_enquiry')) {
    ...
    protected function getList() {
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link('catalog/seller_enquiry', 'token=' . $this->session->data['token'] . $url, true)
        );

        $data['delete'] = $this->url->link('catalog/seller_enquiry/delete', 'token=' . $this->session->data['token'] . $url, true);
        ...
        $results_total = $this->model_catalog_seller_enquiry->getTotalEnquiries($filter_data);

        $results = $this->model_catalog_seller_enquiry->getEnquiriesList($filter_data);

        foreach ($results as $result) {

            $data['seller_enquiries'][] = array(
                'enquiry_id'            => $result['enquiry_id'],
                'seller_name'           => $result['seller_username'],
                ...
                'view'                  => $this->url->link('catalog/seller_enquiry/view', 'token=' . $this->session->data['token'] . '&enquiry_id=' . $result['enquiry_id'] . $url, true)
            );
        }
        ...
        $pagination->url = $this->url->link('catalog/seller_enquiry', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
        ...
        $this->response->setOutput($this->load->view('catalog/seller_enquiry_list', $data));
    }
}
