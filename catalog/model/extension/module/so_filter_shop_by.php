...
class ModelExtensionModuleSofiltershopby extends Model {
	...
	public function getOptions(){
		...
		$sql = 'SELECT o.*, od.name AS option_name FROM `'.DB_PREFIX.'option` AS o LEFT JOIN `'.DB_PREFIX.'option_description` AS od ON o.option_id = od.option_id WHERE o.type IN ('.$type.') AND od.language_id = "1" ' ;
		$query = $this->db->query($sql);
		return $query->rows;
	}
	...
	public function getAllManufacturerId($product_id) {
		$product_id = implode(",",array_map('intval',$product_id));
		$query = $this->db->query("SELECT DISTINCT p.manufacturer_id, m.name AS manu_name, m.image AS manu_image FROM `" . DB_PREFIX . "product` AS p LEFT JOIN `".DB_PREFIX."manufacturer` AS m ON p.manufacturer_id = m.manufacturer_id WHERE p.product_id IN (" .$product_id. ") AND p.manufacturer_id != '0' ");
		$manu_data = array();
		foreach($query->rows as $item)
		{
